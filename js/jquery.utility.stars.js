...
;(function ($, window, document, undefined) {
	...
	$.widget('utility.starRating', {
		...
		_create: function() {
			...
			o.is_select = o.input_type == 'select';
			this._form = $(this.element).closest('form');
			this._select = o.is_select 
				? $('select', this.element) 
				: null;

			this._rboxes = o.is_select 
				? $('option', this._select) 
				: $(':radio', this.element);
			...
			this._stars = this._rboxes.map(function(i)
			{
				var el = {
					value: this.value,
					title: (o.is_select ? this.text : this.title) || this.value,
					is_default: (o.is_select && this.defaultSelected) || this.defaultChecked
				};
				...
				if (i==0) {
					...
					o.id2val = [];
					o.id2title = [];
					o.name = o.is_select ? self._select.get(0).name : this.name;
					o.disabled = o.disabled || (o.is_select 
						? $(self._select).attr('disabled') 
						: $(this).attr('disabled')
					);
				}
			...
			// Remove old content
			o.is_select ? this._select.remove() : this._rboxes.remove();

			// Append Stars interface
			this._cancel = $('<div/>').addClass(o.cancel_class)
				.append($('<a/>').attr('title', o.show_titles ? o.cancel_title : '').text(o.cancel_value));
			...
			this._cancel.bind('click.stars', function(e) {
				if (!o.force_select && (o.disabled || o.value == o.cancel_value)) 
					return false;
			...
			function fill_to(index, hover) {
				if (index != -1) {
					...
					self._show_cap(o.id2title[index]);
				}
				else fill_none();
			};
			function fill_none() {
		...
		select: function(val) {
			var o = this.options, e = (val == o.cancel_value) 
				? this._cancel 
				: this._stars.eq(o.val2id[val]);
