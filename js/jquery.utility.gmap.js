...
;(function ($, window, document, undefined) {
	...
	$.widget("utility.gmap", {
		...
		options: {
			...
			disable_control: false,             // Master control switch
			...
			bubble_min_width: 20,               // Bubble (InfoWindow) minimum width
			bubble_padding: 0,                  // Bubble (InfoWindow) padding

			map_type_id: 'ROADMAP',             // Apply map style identifier (Default: ROADMAP)

			default_style: null,                // Apply map styles 
		...
		_create: function () {
			...
			if (this.options.disable_control) {
				this.options.allow_drag = false;
				this.options.allow_scrollwheel = false;
				this.options.alow_dbl_click_zoom = false;
			}
			...
			var map_options = {
				...
				mapTypeId: google.maps.MapTypeId[this.options.map_type_id]
			};

			if (this.options.default_style)
				map_options.mapTypeId = this.options.default_style_id;
			...
			if (this.options.start_address)
				mapObj.address = this.options.start_address;

			this.__call('map', mapObj);
			this.__call('defaults', map_defaults);
			
			// Styles
			if (this.options.default_style)
				this.defineStyle(this.options.default_style_id, this.options.default_style_name, this.options.default_style);
		...
		__call: function(name, options) {
			var callObj = {};
			callObj[name] = options;
			return this.element.gmap3(callObj);
		},
		...
		setCenter: function(lat, lng) {
			this.__get('map').setCenter(this.getLatLngObject(lat, lng));
		},
		...
		defineStyle: function(id, name, styles) {
			var styleObj = {
				id: id,
				options: { name: name },
				styles: [styles]
			...
			this.__call('styledmaptype', styleObj);
		},
		...
		getObjectArrayFromAddressString: function(string, options) {
			options = $.extend(true, { callback: null }, options);
			this.__call('getlatlng', $.extend(true, {
				address: string, 
		...
		getObjectFromAddressString: function(string, options) {
			options = $.extend(true, { callback: null }, options);
			this.getObjectArrayFromAddressString(string, {
				callback: function(address) {                
					found_address = (address && address[0]) 
						? address[0] 
						: null;
					options.callback && options.callback(found_address);
				}
			});
		},

		// Lookup, Returns object {lat,lng}
		getLatLngFromAddressString: function(string, options) { var self = this;
			options = $.extend(true, { callback: null }, options);
			this.getObjectFromAddressString(string, {
				callback: function(address) {
					var result = (address) 
						? self.getLatLngFromAddressObject(address) 
						: { lat:false, lng: false }; 
		...
		getValueFromAddressObjectArray: function(addressObj, value, options) {
			var self = this;
			options = $.extend(true, {set:0}, options);
			var set = options.set;      

			var result = this.getValueFromAddressObject(addressObj[set]);

			// If no result, check other data sets
			if (!result && addressObj[set+1]) {
				options.set = set + 1;
				return self.getValueFromAddressObjectArray(addressObj, value, options);
			}
		...
		getValueFromAddressObject: function(addressObj, value, options) {
			...
			for (i = 0; i < addressObj.address_components.length; i++) {
				for (j = 0; j < addressObj.address_components[i].types.length; j++) {
					...
					if(addressObj.address_components[i].types[j] == value)
						result = addressObj.address_components[i].short_name;
				}
			}
		...
		alignToAddressString: function(addressString, options) { var self = this;
			options = $.extend(true, {
				zoom: null, 
				callback: null
			}, options);

			this.getObjectFromAddressString(addressString, {
				callback: function(address) {
					if (address) 
						self.alignToAddressObject(address, options.zoom);
		...
		alignToAddressObject: function(addressObj, zoom) {
			var latlng = this.getLatLngFromAddressObject(addressObj);
		...
		updateMarkers: function(markersArray, markerEvents, markerOptions, cleanupMarkers) { var self = this;
			var currentMarkers = this.getMarkers();
			var proposedMarkerIds = $.map(markersArray, function(marker) { return marker.id; });
			var currentMarkerIds = $.map(currentMarkers, function(marker) { return marker.id; });

			if (currentMarkers.length <= 0)
				return this.addMarkers(markersArray, markerEvents, markerOptions);

			// Remove non supplied
			if (cleanupMarkers) {
				$.each(currentMarkerIds, function(key, markerId) {
					if ($.inArray(markerId, proposedMarkerIds) == -1) {
						self.clearMarkerById(markerId);
					}
				});
			}
			...
			$.each(markersArray, function(key, marker) {
				if ($.inArray(marker.id, currentMarkerIds) == -1)
					self.addMarker(marker, markerEvents, markerOptions);
			});
		},

		updateMarker: function(marker, markerEvents, markerOptions) {
			if (!marker.id)
				return;
			
			if (!this.getMarkersById(marker.id))
				this.addMarker(marker, markerEvents, markerOptions, false);
		},

		// Array format [{latlng:[1,1], id: 'address_1', data: { zip:0000, city:"SYDNEY" }}]
		// markerOptions example: { animation: google.maps.Animation.DROP }
		addMarkers: function(markersArray, markerEvents, markerOptions) {
			var self = this;
			
			// Events
			var markerEvents = $.extend(true, {
			...
			// Clustering
			markerOptions = $.extend(true, markerOptions, this.getClusterRules());

			this.__call('marker', markerOptions);
		},

		// marker: {latlng:[1,1], id: 'address_1', data: { zip:0000, city:"SYDNEY" }}
		addMarker: function(marker, markerEvents, markerOptions) {
			this.addMarkers([marker], markerEvents, markerOptions);
		},

		addMarkerFromLatLng: function(lat, lng, id, options) {
			var markerArray = { lat:lat, lng:lng, id:id };
			var markerEvents = (options && options.markerEvents) ? options.markerEvents : { };

			if (options && options.marker_data)
				markerArray.data = options.marker_data;

			this.addMarker(markerArray, markerEvents);
		},

		addMarkerFromAddressString: function(addressString, id, options) { var self = this;
			options = $.extend(true, {
				zoom: null, 
				callback: null
			}, options);
			...
			this.getObjectFromAddressString(addressString, {
				callback: function(address) {
					if (address) {
						self.alignToAddressObject(address, options.zoom);
						self.addMarkerFromAddressObject(address);
					}
		...
		addMarkerFromAddressObject: function(address_object, id, options) {
			var latlng = this.getLatLngFromAddressObject(address_object);
			this.addMarkerFromLatLng(latlng.lat, latlng.lng, id, options);
		},

		getMarkers: function() {
			return this.__get('marker', { full:true, all:true });
		},
		...
		getVisibleMarkers: function() {
			...
			for (var i = markers.length, bounds = this._map.getBounds(); i--;) {
				if (bounds.contains(markers[i].object.getPosition())) {
					// TODO:
					alert('found '+i);
				}
			}
		...
		clearMarker: function(marker) {
			...
			return this.clearMarkerFromId(marker.id);
		},

		clearMarkerById: function(id) {
			this.__call('clear', {name:'marker', id:id});
		},

		// Clusters
		//

		getClusterRules: function() {
			return (!$.isEmptyObject(this._cluster_rules)) 
				? { cluster: this._cluster_rules } 
				: { };
		},

		addClusterRule: function(number, width, height, css_class) {
		...
		showBubbleAtMarker: function(marker, content) {
			var position = marker.getPosition();
			this.showBubbleAtLatLng(position.lat(), position.lng(), content);

			infowindow = this.__get('infowindow');
			infowindow.open(this._map, marker);
		},

		showBubbleAtLatLng: function(lat, lng, content) {
			this.__call('infowindow', {
				latLng: this.getLatLngObject(lat,lng),
				options: { 
					content: content,
		...
		showOverlayAtMarker: function(marker, content) {
			var position = marker.getPosition();
			this.showOverlayAtLatLng(position.lat(), position.lng(), content);
		},

		showOverlayAtLatLng: function(lat, lng, content) {
			this.__call('overlay', {
				latLng: this.getLatLngObject(lat,lng),
				content: content
			});
		...
		showCircleAtAddressObject: function(address_object, radius) {
			var latlng = this.getLatLngFromAddressObject(address_object);
			this.showCircleAtLatLng(latlng.lat, latlng.lng, radius);
		},

		showCircleAtMarker: function(marker, radius) {
			var position = marker.getPosition();
			this.showCircleAtLatLng(position.lat(), position.lng(), radius);
		},

		showCircleAtLatLng: function(lat, lng, radius) {
			this.__call('circle', {
				options: {
					center: [lat, lng],
